...
function ChatContent() {
  ...
  return (
    <div className="h-[calc(100dvh-6.5rem)] md:h-[calc(100vh-6rem)] flex flex-col lg:flex-row gap-4 lg:gap-6 relative min-h-0">
      {/* Left Column: Knowledge Base Selector (Desktop) */}
      <div className="hidden lg:flex w-64 shrink-0 glass-panel p-4 flex-col border border-white/[0.06] bg-[#141724]/70">
      ...
      <div className="flex-1 flex flex-col glass-panel rounded-2xl border border-white/[0.06] bg-[#141724]/60 min-w-0 overflow-hidden">
        {/* Chat Header Bar */}
        <div className="h-14 sm:h-16 px-3 sm:px-6 border-b border-white/[0.06] flex items-center justify-between bg-[#141724]/80 backdrop-blur-md shrink-0 gap-2">
          {/* Desktop KB Title */}
          <div className="hidden lg:flex items-center gap-3 min-w-0">
            <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />
            <div className="min-w-0">
              <div className="text-sm font-bold text-white tracking-tight truncate">{activeKbName}</div>
              <div className="text-[11px] text-slate-400">Grounded Generation Active</div>
            </div>
          </div>

          {/* Mobile KB Selector Dropdown */}
          <div className="lg:hidden flex items-center gap-2 min-w-0 flex-1">
            ...
            <select
              ...
              className="text-xs font-bold text-white bg-[#0e101a] border border-white/[0.1] rounded-lg px-2.5 py-1.5 outline-none max-w-[170px] truncate"
            >
              {knowledgeBases.map((kb) => (
          ...
          <div className="flex items-center gap-2 shrink-0">
            {messages.length > 0 && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold bg-white/[0.04] hover:bg-white/[0.08] text-slate-300 hover:text-white border border-white/[0.08] transition-colors"
            ...
            <div className="hidden sm:flex text-xs font-semibold px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 items-center gap-1.5">
        ...
        <div className="flex-1 overflow-y-auto p-3.5 sm:p-6 space-y-4 sm:space-y-6">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center p-4 sm:p-8">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center mb-4 text-indigo-400 shadow-lg shadow-indigo-500/10">
              ...
              <div className="mt-6 sm:mt-8 grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-2.5 max-w-lg w-full">
                {(suggestedQuestions.length > 0
                  ...
                  <button
                    ...
                    className="p-2.5 sm:p-3 text-left rounded-xl bg-white/[0.02] border border-white/[0.06] hover:border-indigo-500/30 hover:bg-white/[0.04] text-xs text-slate-300 transition-colors line-clamp-2"
                    title={suggested}
                  >
          ...
          {messages.map((msg) => (
            ...
            >
              <div
                className={`max-w-[92%] sm:max-w-[80%] rounded-2xl p-3.5 sm:p-5 ${
                  msg.role === "user"
              ...
              >
                ...
                {msg.citations && msg.citations.length > 0 && (
                  <div className="mt-3 sm:mt-4 pt-3 border-t border-white/[0.08] flex flex-wrap items-center gap-1.5 sm:gap-2">
                    <span className="text-[10px] sm:text-[11px] font-semibold text-slate-400">Sources:</span>
                    {msg.citations.map((cit) => (
                      <button
                        ...
                        className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 rounded-lg text-[11px] sm:text-xs font-semibold bg-white/[0.06] hover:bg-indigo-500/20 hover:border-indigo-500/40 border border-white/[0.08] text-slate-200 transition-all"
                      >
                        <FileText className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-400" />
                        <span>[{cit.citation_index}]</span>
                        <span className="max-w-[100px] sm:max-w-[120px] truncate">{cit.document_name}</span>
                      </button>
                    ))}
                  </div>
                )}
                    ...
                    <div className="flex items-center gap-3">
                      {msg.retrieval_metadata && (
                        <button
                          ...
                          className="flex items-center gap-1.5 text-indigo-400 hover:text-indigo-300 font-semibold transition-colors text-[11px] sm:text-xs"
                        >
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="p-3.5 sm:p-4 rounded-2xl bg-[#1c2136] border border-white/[0.08] flex items-center gap-3 text-slate-300 text-xs font-medium">
        ...
        {/* Input Bar */}
        <div className="p-3 sm:p-4 border-t border-white/[0.06] bg-[#141724]/90">
          <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-2.5 rounded-2xl bg-[#1a1e30] border border-white/[0.08] focus-within:border-indigo-500/50 transition-colors shadow-inner">
            <input
              type="text"
              placeholder={
                selectedKb
              ...
              className="flex-1 bg-transparent px-2 sm:px-3 py-1 text-xs sm:text-sm text-slate-100 placeholder:text-slate-400 outline-none min-w-0"
            />
            <button
              onClick={handleSend}
              disabled={!input.trim() || !selectedKb || loading}
              className="px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 disabled:opacity-30 disabled:pointer-events-none text-white shadow-md shadow-indigo-600/20 transition-all flex items-center gap-1.5 sm:gap-2 shrink-0"
            >
        ...
        {(selectedCitation || showRetrieval) && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-x-0 bottom-0 top-16 z-50 lg:static lg:inset-auto lg:top-auto lg:h-full lg:w-[380px] lg:shrink-0 glass-panel border border-white/[0.1] bg-[#141724] lg:bg-[#141724]/85 flex flex-col overflow-hidden shadow-2xl rounded-t-3xl lg:rounded-2xl"
            >
            {/* Header */}
